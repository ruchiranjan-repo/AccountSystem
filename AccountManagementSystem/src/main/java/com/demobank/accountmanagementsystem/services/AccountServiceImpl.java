...
public class AccountServiceImpl implements AccountService {
	...
	public List<AccountDTO> fetchAccountsByIfscCode(String ifscCode) throws AccountNumberNotFoundException {
		...
		if (CollectionUtils.isEmpty(accounts)) {
			...
			throw new AccountNumberNotFoundException(123L);
		}
	...
	public void initiateTransaction(TransactionDTO transactionDto) throws TransactionFailedException {
		...
		if (fromBalance.compareTo(amount) == 1) {
			...
			Account a1=mapper.mapAccountDTOToAccount(fromAccount);
					System.out.println(a1.getCustomer().getCustomerId());
			accountRepository.save(a1);
			Account s=mapper.mapAccountDTOToAccount(benificiaryAccount);
			System.out.println(s.getCustomer().getCustomerId());
